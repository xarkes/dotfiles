...
beautiful.init(gears.filesystem.get_configuration_dir() .. "xarkes/theme.lua")
...
-- {{{ Menu (Disabled because never used)
...
-- Battery widgets
mybat0 = battery_widget { adapter = "BAT0", ac_prefix = "ﮤ", battery_prefix = "0 "  }
mybat1 = battery_widget { adapter = "BAT1", ac_prefix = "ﮤ", battery_prefix = "1 "  }

-- CPU and RAM widgets
mycpu = wibox.widget.textbox()
vicious.register(mycpu, vicious.widgets.cpu, "<span color=\"#c8e9a0\">CPU $1% </span>", 3)
myram = wibox.widget.textbox()
vicious.register(myram, vicious.widgets.mem, "<span color=\"#efc7c2\">RAM $1% </span>", 3)
...
awful.screen.connect_for_each_screen(function(s)
    ...
    awful.tag({ "  ", "爵 ", " ﲵ", " ", " ﲵ", " ﲵ", " ﲵ", " ", "" }, s, awful.layout.layouts[2])
    --awful.tag.add("First tag", {
    --        icon           = gears.filesystem.get_configuration_dir() .. "icons/github.svg",
    ...
    -- Do not show application top bar
    s.mytasklist = awful.widget.tasklist(s, awful.widget.tasklist.filter.currenttags, tasklist_buttons, { align = "center" })
